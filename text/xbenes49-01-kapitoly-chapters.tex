...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.8\textwidth]{obrazky-figures/roomsegments.png}
\caption{Structure of a~skin. \cite{PIROperationalManual}\label{fig:roomsegments}}
\end{center}    
\end{figure}
...
discovered experimentally minimizing the sum of the within-segment variances of the recorded data.

\begin{figure}[!ht]
\begin{equation}
\Psi(x) = e^{-\frac{x^2}{2}} cos(5x)
\end{equation}
\caption{Morlet {\it mexican hat} Wavelet.}
\end{figure}
...
\end{equation}
\caption{Experimental optimalization of parameters.}
\end{figure}

In next step their distances are evaluated. Distance of two adjoining segments is computed as numerical
...
\end{equation}
\caption{Distance of segments.}
\end{figure}

There comes to the game fuzzification. A designed fuzzy sets corresponds with falling edge (\textbf{F}), stagnating signal (\textbf{S})
and rising edge (\textbf{R}). The E and R are related, they use the same wave form. Both of the curves can be parametrized with
$t$ being tolerance and $x_0$ being center/shift. The fuzzy set arrange is visualized in the figure \ref{fig:fuzzysets} with $t=10$.

\begin{figure}[!ht]
\begin{equation}
\xi_R(x) = \frac{ arctan( 4\cdot (\frac{x - x_0}{t} + 1) )}{\pi} + 0.5
\end{equation}
\caption{Fuzzy set function used for \textbf{R}.}
\end{figure}
...
\xi_L(x) = \xi_R(-x)
\end{equation}
\caption{Fuzzy set function used for \textbf{F}.}
\end{figure}
...
\end{equation}
\caption{Fuzzy set function used for \textbf{S}.}
\end{figure}
...
Classifier is built on the observation ...
