...
Using more sensors brings several advantages: the measuring might be more precise since the person presence
classified by two independent sensors is not only more probable but the computed position
can get more accurate than when using only one sensor.

If more sensors are used it is necessary to merge their space segments matrixes in one.
To do that, a fuzzy logic mechanism {\it Takagi-Sugeno rules} can be used as described in \cite{InsightIntoFuzzyModelling}.
The form of rules is [IF {\it antecendent} THEN {\it succendent}] as shown in the table \ref{table:takagisugeno}.
During the calculation all the antecendentes are evaluated and the most relevant leads to application of corresponding succendent to the output.
...
\end{tabular}
\caption{Takagi-Sugeno rules.\label{table:takagisugeno} \cite{InsightIntoFuzzyModelling}}
\end{table}

%\begin{equation}
%\begin{split}
%R_1 := \mathit{if} (X_1 is A_{11}) \mathit{and} \dots \mathit{and} (X_n is A_{1n}) \mathit{then} \\
%            Y = b_{10} + b_{11}X_1 + \dots + b_{1n}X_n \\
%\dots \\
%R_m := IF X_1 is A_{m1} AND \dots AND X_n is A_{mn} THEN \\
%            Y = b_{m0} + b_{m1}X_1 + \dots + b_{mn}X_n
%\end{split}
%\end{equation}

The algorithm is based on IF-THEN rules (implications), where for input antecedents (conditions) are evaluated all together and the
most relevant is used. The 
