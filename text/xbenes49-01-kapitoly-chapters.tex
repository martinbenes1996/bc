...
Infrared spectrum is also used in astronomy because it can better penetrate clouds of gas and dust than
the visible light. Observing the space in the infrared spectrum gives us much more information than just
visible light: stars behind the particle clouds, planets and moons, since they are not shining, composition
of their athmosphere by observing the way how IR is behaving in it etc.
...
The whole electromagnetic waves are being divided into categories according to their usage by the frequency $f$
or wavelength $\lambda$. With increasing wavelength~$\lambda$ it is gamma, X-rays, ultraviolet (UV),
visible light, infrared (IR) and radio waves. This is called electromagnetic spectrum
and it is shown in the image \ref{fig:spectrum}.
...
Another measurable characteristic is {\it an energy of radiation}. It is linearly dependend on frequency and can be computed as
$$W_e=h\cdot f$$
where $f$ is the frequency of the electromagntic radiation and
$h=6.63\cdot10^{-34}~J\cdot s $
is a Planck constant. \cite{NasaEMSpectrum}

{\it Irradiance} $I$ is the radiant flux per surface. The Stefan-Boltzmann law defines it
for electromagnetic radiation as
$$I = \sigma \cdot T^4$$
where $\sigma = 5.6704\cdot 10^{-8} Wm^{-2}K^{-4}$ is the Stefan-Boltzmann constant and
$T$ is a thermodynamic temperature. 



\section{Radiation processed by organisms}
Many organisms evolved so they could process parts of electromagnetic spectrum. The eyes enables not only
mammals, but also cephalopods and arthropods to see - to sense what we call visible light, but not only -
some insects can even see a part of ultraviolet. Additionally organisms including human often have
receptors in their skin so they can get information about intensity of infrared radiation around them,
so called thermoreceptors. We feel infrared radiation as temperature.


\subsection*{Visible light}
\label{subsection:eye}
Human eyes can process radiation $\lambda \in \langle 380~nm;760~nm \rangle$, it is called visible light, one of the bands
of electromagnetic spectrum. We can see objects by receiving radiation from a light source reflected from
the surface of observed object on our retina. There a biological system composed of light-sensitive cells
rods and cones which propagate the information through a nerve to the brain.

The ray coming to our eye is going through a converging lens, which changes its trajectory aiming to the
most sensitive place on retina (with a lot of the rods and cons) called {\it Fovea centralis}. \cite{LightEyeVision}

\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.4\textwidth]{obrazky-figures/placeholder.jpg}
\caption{Structure of an eye.\label{fig:eye}}
\end{center}    
\end{figure}


\subsection*{Heat}
Same as the visible light, the infrared rays surrounds us during our whole life too. We sense it as heat.
The heat receptors called {\it thermoreceptors} are common amongst the evolved animals, on our body it is
located especially on a surface (in a skin), but also in some organs.

There is a difference between visible light and infrared radiation - processed visible light can come from
a source or reflected from the surface, while the IR can originate only from the primary source - warm item.

Our skin contains two types of thermoreceptors - sensing cold, colder than a body temperature and
hot - hotter than a body temperature. This is already well described, on the other hand the 
evaluation center of these receptors in the brain and its mechanisms is not fully understanded yet
and a matter of current research. \cite{BodilySenses} 

\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.4\textwidth]{obrazky-figures/placeholder.jpg}
\caption{Structure of a skin.\label{fig:skin}}
\end{center}    
\end{figure}

Some animals even use sensing the heat as a primary way how to survive. Several groups of snakes
(pythons, rattlesnakes, boas and others) have special organs under their eyes which can be used
when hunting warm-blooded animals (mouses, rats, rabbits etc.). Using the receptor they can
accuratly estimate the direction and the distance of the prey by its body temperature. \cite{SnakeInfrared}

Blood-eating organisms such as vampire bats or south-american heteroptera {\it Triatoma infestans}
can use IR receptors to look for a warm spot - a vein under the skin. Then they bite on the
right place and lick the blood. 

\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.6\textwidth]{obrazky-figures/python.jpeg}
...
astronome sir Frederick Harschel. He dispersed light by a prism and found out that the temperature of
the light is growing with wavelength, the red light had the highest one.
...
pronounced hypothesis that except the visible light there must be also invisible one which we can
not see. \cite{HerschelLife}
...
the physical aspects that needs to be watched and often regulated is a temperature. There are generally
three types of animals - {\it ectotherms}, {\it endotherms} and {\it mesotherms}.

Ectotherms do not regulate its body temperature and rely on the external source, endotherms keeps it the
same indipendently on the environment, this is called {\it homeothermy}\footnote{Homeothermy is an aspect
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.6\textwidth]{obrazky-figures/thermoregulatory.png}
...
it can rise up to $40^{\circ}C$. To compute the peak wavelength of the radiation emitted by human body
(temperature $37^{\circ}C$ or $310.15~K$) the Wien's displacement law can be used.
$$\lambda_{max}=\frac{b}{T}=\frac{2.8977729 \cdot 10^{-3}}{310.15} = 9.3431~\mu m$$

%\begin{figure}[h!]
%\begin{center}
%\includegraphics[width=0.4\textwidth]{obrazky-figures/placeholder.jpg}
%\caption{Human body wavelength.\label{fig:bodywavelength}}
%\end{center}    
%\end{figure}

%The figure \ref{fig:bodywavelength} shows the measured frequency composition of the IR emitted by human
%body by the ...cite....
...
energy into the electric voltage and send it away via wire to be processed by some hardware or software.
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.3\textwidth]{obrazky-figures/pirscheme.png}
...
The structure of PIR sensor is inspirated by structure of an eye described in subsection
\ref{subsection:eye}. The incoming infrared radiation coming to the sensor is going through
so called {\it Fresnel lens}.

<Description of Fresnel lens>.

\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.5\textwidth]{obrazky-figures/fresnellens.png}
